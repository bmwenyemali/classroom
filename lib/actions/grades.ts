...
export async function getStudentGradeStats(studentId: string) {
  ...
  const averageScore = grades?.reduce((sum, g) => sum + (g.score / g.max_score * 100), 0) / totalGrades || 0
  
  // GPA calculation (4.0 scale)
  const gpa = grades?.reduce((sum, g) => {
...
export async function bulkCreateGrades(grades: Array<{
  student_id: string
  course_id: string
